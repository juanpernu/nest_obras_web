// Elimina de dist los assets de _astro que no referencia NADIE.
//
...
for (const abs of candidatos) {
  ...
  await unlink(abs);
  console.log(`[prune-orphan-assets] eliminado (sin referencias): /${relative(DIST, abs)}`);
  removed++;
}

console.log(
  removed
    ? `[prune-orphan-assets] ${removed} archivo(s), ${bytes.toLocaleString('es-AR')} bytes.`
